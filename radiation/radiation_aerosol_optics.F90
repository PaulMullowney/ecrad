  ...
  subroutine add_aerosol_optics(nlev,istartcol,iendcol, &
    ...
    if (aerosol%is_direct) then
...
#ifdef DEBUG_WARNING
      write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, " The add_aerosol_optics_direct may not be ported to OMPGPU"
#endif
...
#if 1
      write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, " the get_gas computation is not running properly on device. so we do the computation here for now. This needs to be fixed."

      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2)
      do jcol = istartcol,iendcol
         do jlev = 1,nlev
            h2o_mmr(jcol,jlev) = gas%mixing_ratio(jcol,jlev,IH2O)
         end do
      end do
      !$OMP END TARGET TEAMS DISTRIBUTE PARALLEL DO
#else
      write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, " the get_gas computation is not running properly on device so we do the computation on the host."
      !$OMP TARGET UPDATE FROM(gas%mixing_ratio)
      call gas%get(gas, IH2O, IMassMixingRatio, h2o_mmr, istartcol=istartcol)
      !$OMP TARGET UPDATE TO(h2o_mmr)
#endif
      ...
      if (config%do_lw_aerosol_scattering) then
         write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, "do lw_aerosol_scattering"
      endif
      ! Loop over column
#if defined(OMPGPU)
    ...
    !$OMP TARGET UPDATE FROM(od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw)
    !$ACC UPDATE HOST(od_lw, ssa_lw, g_lw, od_sw, ssa_sw, g_sw) ASYNC(1)
    !$ACC WAIT(1)
    s1 = lbound(od_lw,1)
    s2 = lbound(od_lw,2)
    s3 = lbound(od_lw,3)
    ...
    if (config%do_lw_aerosol_scattering) then
       ...
       s2 = lbound(ssa_lw,2)
       s3 = lbound(ssa_lw,3)
       write(nulout,'(a,g0.5,a,i0,a,i0,a,i0)') "ssa_lw=", ssa_lw(s1,s2,s3), " at indices ", s1, " ", s2, " ", s3
       s1 = ubound(ssa_lw,1)
       s2 = lbound(ssa_lw,2)
       s3 = lbound(ssa_lw,3)
       write(nulout,'(a,g0.5,a,i0,a,i0,a,i0)') "ssa_lw=", ssa_lw(s1,s2,s3), " at indices ", s1, " ", s2, " ", s3
       ...
       s2 = lbound(g_lw,2)
       s3 = lbound(g_lw,3)
       write(nulout,'(a,g0.5,a,i0,a,i0,a,i0)') "g_lw=", g_lw(s1,s2,s3), " at indices ", s1, " ", s2, " ", s3
       s1 = ubound(g_lw,1)
       s2 = lbound(g_lw,2)
       s3 = lbound(g_lw,3)
       write(nulout,'(a,g0.5,a,i0,a,i0,a,i0)') "g_lw=", g_lw(s1,s2,s3), " at indices ", s1, " ", s2, " ", s3
    ...
    s2 = lbound(g_sw,2)
    s3 = lbound(g_sw,3)
    write(nulout,'(a,g0.5,a,i0,a,i0,a,i0)') "g_sw=", g_sw(s1,s2,s3), " at indices ", s1, " ", s2, " ", s3
    s1 = ubound(g_sw,1)
    s2 = lbound(g_sw,2)
    s3 = lbound(g_sw,3)
    write(nulout,'(a,g0.5,a,i0,a,i0,a,i0)') "g_sw=", g_sw(s1,s2,s3), " at indices ", s1, " ", s2, " ", s3
...
!#ifdef DEBUG
    write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, " has OpenACC code that is not ported"
!#endif
