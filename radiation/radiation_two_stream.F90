...
!#define DO_DR_HOOK_TWO_STREAM
  ...
  !$omp declare target(calc_two_stream_gammas_lw)
  !$omp declare target(calc_two_stream_gammas_sw)
  !$omp declare target(calc_reflectance_transmittance_lw)
  !$omp declare target(calc_ref_trans_lw)
  !$omp declare target(calc_ref_trans_acc_lw)
  !$omp declare target(calc_ref_trans_lw_OMP_single_level)
  ...
  !$omp declare target(calc_ref_trans_omp_sw)
  !$omp declare target(calc_ref_trans_omp_sw_single_level)
  !$omp declare target(calc_two_stream_gammas_sw_single_band)
  !$omp declare target(calc_reflectance_transmittance_sw_single_band)
contains
...
!NEC$ shortloop
  ...
  ! Calculate the two-stream coefficients gamma1-gamma4 in the
  ! shortwave
  subroutine calc_two_stream_gammas_sw_single_band(jg, mu0, ssa, g, &
       &                               gamma1, gamma2, gamma3)
    ...
    if (lhook) call dr_hook('radiation_two_stream:calc_two_stream_gammas_sw_single_band',0,hook_handle)
#endif
...
#ifdef DO_DR_HOOK_TWO_STREAM
    if (lhook) call dr_hook('radiation_two_stream:calc_two_stream_gammas_sw_single_band',1,hook_handle)
#endif

  end subroutine calc_two_stream_gammas_sw_single_band
...
!NEC$ shortloop
  ...
  ! OpenACC-optimized variant that avoids the local allocation of gamma
  ! coefficients
  subroutine calc_ref_trans_lw_OMP_single_level(jg, ng, &
  ...
  end subroutine calc_ref_trans_lw_OMP_single_level
...
!NEC$ shortloop
  ...
  ! trans_dir_dir is the transmittance of the atmosphere to direct
  ! radiation with no scattering.
  subroutine calc_reflectance_transmittance_sw_single_band(jg, ng, mu0, od, ssa, &
       &      gamma1, gamma2, gamma3, ref_diff, trans_diff, &
       &      ref_dir, trans_dir_diff, trans_dir_dir)
    ...
    if (lhook) call dr_hook('radiation_two_stream:calc_reflectance_transmittance_sw_single_band',0,hook_handle)
#endif
...
!NEC$ shortloop
...
#ifdef DO_DR_HOOK_TWO_STREAM
    if (lhook) call dr_hook('radiation_two_stream:calc_reflectance_transmittance_sw_single_band',1,hook_handle)
#endif

  end subroutine calc_reflectance_transmittance_sw_single_band
  ...
  ! coefficients
  subroutine calc_ref_trans_omp_sw(jg, ng, nlev, mu0, od, ssa, &
    &      asymmetry, ref_diff, trans_diff, &
    &      ref_dir, trans_dir_diff, trans_dir_dir)
  ...
  end subroutine calc_ref_trans_omp_sw

  !---------------------------------------------------------------------
  ! OpenMP-optimized variant that avoids the local allocation of gamma
  ! coefficients
  subroutine calc_ref_trans_omp_sw_single_level(jg, ng, mu0, od, ssa, &
    &      asymmetry, ref_diff, trans_diff, &
    &      ref_dir, trans_dir_diff, trans_dir_dir)
  ...
  end subroutine calc_ref_trans_omp_sw_single_level
