...
#include "radiation_delta_eddington.h"
  ...
  subroutine solver_mcica_omp_sw(nlev,istartcol,iendcol, &
    ...
    use radiation_two_stream, only     : calc_two_stream_gammas_sw, calc_two_stream_gammas_sw_single_band, &
         &                               calc_reflectance_transmittance_sw, calc_reflectance_transmittance_sw_single_band, &
         &                               calc_ref_trans_omp_sw, calc_ref_trans_omp_sw_single_level
    
    use radiation_adding_ica_sw, only  : adding_ica_sw, adding_ica_sw_omp
    ...
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) PRIVATE(cos_sza, gamma1, gamma2, gamma3, od_cloud_new, od_total, ssa_total, g_total, scat_od, jcol, jg, jlev) FIRSTPRIVATE(istartcol, iendcol, ng, nlev) THREAD_LIMIT(512)
    do jcol = istartcol,iendcol
       do jg = 1, ng
          ! Only perform calculation if sun above the horizon
          if (single_level%cos_sza(jcol) > 0.0_jprb) then
             ...
             if (.not. config%do_sw_delta_scaling_with_gases) then
                ...
                call calc_ref_trans_omp_sw(jg, ng, nlev, &
                ...
                do jlev = 1,nlev
                   ...
                   call calc_two_stream_gammas_sw_single_band(jg, &
                        &  cos_sza, ssa_total, g_total, &
                        &  gamma1, gamma2, gamma3)

                   call calc_reflectance_transmittance_sw_single_band(jg, ng, &
             ...
             if (flux%cloud_cover_sw(jcol) >= config%cloud_fraction_threshold) then
                ...
                do jlev = 1,nlev
                   ...
                   if (frac(jlev,jcol) >= config%cloud_fraction_threshold) then
                      ...
                      ! each model level
                      call calc_ref_trans_omp_sw_single_level(jg, ng, &
...
#ifdef WORKAROUND_NESTED_PARALLEL_MCICA_OMP_SW
...
#ifndef WORKAROUND_NESTED_PARALLEL_MCICA_OMP_SW
          !$OMP PARALLEL DO REDUCTION(+:sum_dn_diffuse, sum_dn_direct, sum_up)
#endif
          do jg = 1,ng
