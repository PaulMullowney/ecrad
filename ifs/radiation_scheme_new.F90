SUBROUTINE RADIATION_SCHEME_NEW &
...
! !$OMP&         PEMIS_OUT, PLWDERIVATIVE,PSWDIRECTBAND, PSWDIFFUSEBAND)
!!$OMP TARGET DATA MAP(PRESENT,ALLOC: PRE_LIQ) IF(present(PRE_LIQ))
!!$OMP TARGET DATA MAP(PRESENT,ALLOC: PRE_ICE) IF(present(PRE_ICE))
!!$OMP TARGET DATA MAP(PRESENT,ALLOC: PCLOUD_OVERLAP)IF(present(PCLOUD_OVERLAP))
#endif
...
#ifdef DEBUG_WARNING
  write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, " WARNING has OpenACC code that has not been ported"
#endif
...
END SUBROUTINE RADIATION_SCHEME_NEW
