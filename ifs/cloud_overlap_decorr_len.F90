...
IF (KDECOLAT == 0) THEN
  ...
  ENDIF
#ifdef DEBUG_WARNING
  write(nulout,'(a,a,a,i0,a)') "    ", __FILE__, " : LINE = ", __LINE__, " WARNING. Is this right?"
#endif
  IF (PRESENT(PDECORR_LEN_RATIO)) THEN
    PDECORR_LEN_RATIO = RDECORR_CW / RDECORR_CF
  ENDIF
...
ELSE
  ...
  IF (KDECOLAT == 1) THEN
    ...
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO PRIVATE(ZABS_LAT_DEG)
    ...
    !$OMP END TARGET TEAMS DISTRIBUTE PARALLEL DO
  ELSE ! KDECOLAT == 2
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO PRIVATE(ZCOS_LAT)
    !$ACC PARALLEL DEFAULT(NONE) PRESENT(PGEMU, PDECORR_LEN_EDGES_KM) ASYNC(1)
