...
if [[ $staging ]]; then
    ...
    module load amd-staging/2025-06-13
    #module load amd-staging
else
    module load rocm-afar-drop/6.2.0
fi
module list
...
if [[ $cmake ]]; then
	...
	cmake -B ${BUILD_DIR} -S . -DENABLE_BITIDENTITY_TESTING=ON -DCMAKE_BUILD_TYPE=RELEASE -DENABLE_ACC=${ACC} -DENABLE_OMP=${OMP} -DENABLE_GPU=ON -DENABLE_ROCTX=ON -DECBUILD_Fortran_FLAGS="-O3 --offload-arch=${arch} -fopenmp -fPIC -DDEBUG_WARNING -DWORKAROUND_NESTED_PARALLEL_MCICA_OMP_LW -DWORKAROUND_NESTED_PARALLEL_MCICA_OMP_SW -DWORKAROUND_NESTED_PARALLEL_LW_DERIVATIVES -DWORKAROUND_NESTED_PARALLEL_FLUX" -DENABLE_SINGLE_PRECISION=ON -DENABLE_DOUBLE_PRECISION=ON -DNETCDF_ROOT=$HOME/ecmwf/install/netcdf/amd-staging -Decbuild_ROOT=${ECBUILD_PATH} -Dfiat_ROOT=${FIAT_INSTALL_PATH}/amd-staging${mpi_ext}/ -DCMAKE_VERBOSE_MAKEFILE=ON -DCMAKE_EXE_LINKER_FLAGS_INIT="-lflang_rt.hostdevice --offload-arch=${arch} -fopenmp -O3" -DBUILD_SHARED_LIBS=OFF
    else
	#  -DDEBUG_CORRECTNESS_RADIATION
